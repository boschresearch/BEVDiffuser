# This source code is from LayoutDiffusion
...
class LayoutTransformerEncoder(nn.Module):
    def __init__(
        ...
        if 'obj_name' in self.used_condition_types:
            self.obj_name_embedding = nn.Linear(1024, hidden_dim)
        ...
        for resolution in self.resolution_to_attention:
            ...
            if self.use_3d_bbox:
                # 3d bbox: (x0, y0, z0, x_size, y_size, z_size, yaw, vx, vy) 
                # ego coordinate
                self.image_patch_bbox_embedding['resolution{}'.format(resolution)] = torch.FloatTensor(
                    [(interval * j + interval * 0.5 + 0.5, (interval * i + interval * 0.5 + 0.5) * -1, 0, interval, interval, 0, 0, 0, 0) for i in range(resolution) for j in range(resolution)],
                ).cuda()  # (L, 9)
            else:
                # 2d bbox: (x0, y0, x1, y1)
                self.image_patch_bbox_embedding['resolution{}'.format(resolution)] = torch.FloatTensor(
    ...
    def convert_to_fp16(self):
        ...
        if 'obj_name' in self.used_condition_types:
            self.obj_name_embedding.to(th.float16)
        if 'obj_bbox' in self.used_condition_types:
    ...
    def forward(self, obj_class=None, obj_bbox=None, obj_mask=None, is_valid_obj=None, image_patch_bbox=None, obj_name=None):
        ...
        if 'obj_name' in self.used_condition_types:
            obj_name_embedding = self.obj_name_embedding(obj_name)
            if xf_in is None:
                xf_in = obj_name_embedding
            else:
                xf_in = xf_in + obj_name_embedding
            if 'obj_class' in self.used_condition_types:
                outputs['obj_name_embedding'] = obj_name_embedding.permute(0, 2, 1)
            else:
                outputs['obj_class_embedding'] = obj_name_embedding.permute(0, 2, 1)
