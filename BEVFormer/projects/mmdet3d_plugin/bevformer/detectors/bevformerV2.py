...
class BEVFormerV2(MVXTwoStageDetector):
    ...
    def obtain_bev(self, img=None, img_metas=None, **kwargs):
        ...
        for ind, t in enumerate(img_metas.keys()):
            img_dict[t] = img[:, ind, ...]
        return self.obtain_history_bev(img_dict, img_metas)[0]
    ...
    def forward_test(self, img_metas, img=None, only_bev=False, given_bev=None, return_eval_loss=False, **kwargs):
        ...
        new_prev_bev, bbox_results = self.simple_test(img_metas[0], img[0], prev_bev=None, only_bev=only_bev, given_bev=given_bev, **kwargs)
        if only_bev:
            return new_prev_bev
        return bbox_results
    ...
    def simple_test(self, img_metas, img=None, prev_bev=None, only_bev=False, given_bev=None, rescale=False, **kwargs):
        ...
        if only_bev:
            new_prev_bev = self.pts_bbox_head(img_feats, img_metas, prev_bev, only_bev=only_bev)
            return new_prev_bev, None
