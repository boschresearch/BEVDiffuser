...
class BEVFormer(MVXTwoStageDetector):
    ...
    def obtain_bev(self, img=None, img_metas=None, **kwargs):
        if isinstance(img, list):
            return self.forward_test(img_metas, img, only_bev=True, **kwargs)
        return self.obtain_history_bev(img, img_metas)
    ...
    def simple_test_pts(self, x, img_metas, prev_bev=None, given_bev=None, rescale=False):
        """Test function"""
        outs = self.pts_bbox_head(x, img_metas, prev_bev=prev_bev, given_bev=given_bev)

        for key, value in outs.items():
            if isinstance(value, torch.Tensor):
                outs[key] = value.detach()
    ...
    def simple_test(self, img_metas, img=None, prev_bev=None, given_bev=None,rescale=False, **kwargs):
        """Test function without augmentaiton."""
        img_feats = self.extract_feat(img=img, img_metas=img_metas)

        bbox_list = [dict() for i in range(len(img_metas))]
        new_prev_bev, bbox_pts = self.simple_test_pts(
            img_feats, img_metas, prev_bev, given_bev, rescale=rescale)
        for result_dict, pts_bbox in zip(bbox_list, bbox_pts):
            result_dict['pts_bbox'] = pts_bbox
        return new_prev_bev, bbox_list
    
    def simple_test_bev(self, img_metas, img=None, prev_bev=None):
        img_feats = self.extract_feat(img=img, img_metas=img_metas)
        return self.pts_bbox_head(img_feats, img_metas, prev_bev, only_bev=True)
    
    def simple_test_loss(self, img_metas, img=None, prev_bev=None, given_bev=None, **kwargs):
        img_feats = self.extract_feat(img=img, img_metas=img_metas)
        outs = self.pts_bbox_head(img_feats, img_metas, prev_bev=prev_bev, given_bev=given_bev)
        losses = self.pts_bbox_head.loss(
            gt_bboxes_list=kwargs['gt_bboxes_3d'],
            gt_labels_list=kwargs['gt_labels_3d'],
            preds_dicts=outs,
            img_metas=img_metas)
